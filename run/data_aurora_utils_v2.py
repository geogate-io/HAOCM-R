...
class AuroraDataset(Dataset):
    ...
    """
    def __init__(self, data_path: str, output_path: str, start_date: str, end_date: str, time_delta: int, variables: Dict[str, Dict[str, str]], import_vars: Dict[str, Dict[str, float]] = None) -> None:
        ...
        self.len = len(self.time_array) - 1 
    ...
    def __blend__(self, da: xr.DataArray, var: np.ndarray, name: str, scale: float = 1.0, add_offset: float = 0.0, missing_value=1.0e20) -> xr.DataArray:
        ...
        da_new = xr.DataArray(
            ...
            coords=da.coords.drop_vars(['time']),
            name=name
        )

        # Apply scale and offset
        da_new_padded_ext = da_new * scale + add_offset
    ...
    def __getbatch__(self, idxs: List[int], remove_south_pole=False, has_import=False) -> Batch:
        ...
        for idx in idxs:
            ...
            # Check if we have previous Aurora prediction to use as input
            file_name = f"pred_{time_str}.nc"
            if os.path.exists(os.path.join(self.output_path, file_name)):
                file_list.append(os.path.join(self.output_path, file_name))
                #ds = xr.open_dataset(file_list[-1], engine="netcdf4").squeeze(['batch', 'step']).drop_vars(["lsm", "slt"]).rename(
                ds = xr.open_dataset(file_list[-1]).squeeze(['batch', 'step']).drop_vars(["lsm", "slt"]).rename(
                        {
                            "2t": "t2m", 
                            "10u": "u10", 
                            "10v": "v10",
            ...
            # Use ERA5 data if we don't have prediction available
            else:
                file_list.append(os.path.join(self.data_path, f"data_{time_str}.nc"))
                #ds = xr.open_dataset(file_list[-1], engine="netcdf4")
                ds = xr.open_dataset(file_list[-1])
                # Accumulated variables, convert from per hour to per second
                for name in ds.data_vars:
                    if name in ["ssr", "str", "ssrd", "strd", "tp"]:
                        print(f"{name} apply unit conversion")
                        ds[name] = ds[name]/3600.0
        ...
        if "static" in self.variables.keys():
            for key, val in self.variables["static"].items():
                if val not in ds.data_vars:
                    #ds[val] = xr.open_dataset(os.path.join(self.data_path, f"data_{time_str}.nc"), engine="netcdf4")[val]
                    ds[val] = xr.open_dataset(os.path.join(self.data_path, f"data_{time_str}.nc"))[val]
                    ds[val] = ds[val].expand_dims(time=ds.time)
...
def batch_to_dataset(batch) -> xr.Dataset:
    ds = xr.Dataset(
        data_vars = {
            ...
            **{key: (("batch", "step", "latitude", "longitude"), _np(value)) for key, value in batch.surf_vars.items()},
            **{key: (("batch", "step", "level", "latitude", "longitude"), _np(value)) for key, value in batch.atmos_vars.items()},
        },
        coords={
